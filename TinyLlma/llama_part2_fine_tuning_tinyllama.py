# -*- coding: utf-8 -*-
"""llama_part2_Fine_Tuning_Tinyllama.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1TOD4960ythSAg2E5S7Hna0uu-ITR901g
"""
...
# %cd "/content/gdrive/MyDrive/Colab Notebooks/llm2"
...
!pip install -U pandas==2.2.2 numpy==2.0.2 scipy==1.14.1 accelerate==1.6.0 peft==0.15.2 bitsandbytes==0.45.5 transformers==4.51.3 trl==0.16.1 datasets==3.5.0 tensorboard==2.19.0
    ...
    # numpy 1.24.4 + transformers 4.36.x + datasets 2.16.x
...
# !pip uninstall -y numpy==1.26.4
# !pip install -U numpy==1.24.4

# !pip install -U Numpy==1.24.4 pandas==2.2.2 scipy==1.14.1
...
dataset_name = "Ljk0501/kopo_healthcare_exercise_dataset"
...
#     output = dataset['output']
#     text = f"### Instruction:\n{instruction}\n\n### Input:\n{input_}\n\n### Output:\n{output}"
#     # 수정: dataset 자체를 바꾸지 않고 새로운 딕셔너리 리턴
#     return {"text": text}
...
# 결과를 새 변수에 할당하거나 기존 변수를 업데이트
# datasetCommon_converted = datasetCommon.map(convert_to_alpaca_format, remove_columns=datasetCommon.column_names)

datasetCommon = datasetCommon.map(convert_to_alpaca_format,remove_columns=['instruction', 'input', 'output'])
# #datasetCommon = datasetCommon.map(convert_to_alpaca_format, remove_columns=['text'])
# #
...
#     sample['messages'] = sample['messages']+ [EOS_TOKEN]
#     return sample
...
def prompt_eos(sample):
    # 마지막 메시지(assistant)의 content에 EOS_TOKEN 추가
    if isinstance(sample['messages'], list) and len(sample['messages']) > 0:
        # 마지막 메시지가 assistant 메시지인지 확인
        last_message = sample['messages'][-1]
        if isinstance(last_message, dict) and last_message.get('role') == 'assistant':
            last_message['content'] = last_message['content'] + EOS_TOKEN
    return sample
...
training_params = TrainingArguments(
    ...
    output_dir= "/content/gdrive/MyDrive/Colab Notebooks/llm/llama_3.0_8B",
...
#클로드 방법 증명
def extract_response_tinyllama(instruction, input_text="", system_message="너는 전문가 AI 비서야. 항상 답변 마지막에 'hkcode 답변이었습니다'라고 말해야 해."):
    """
    TinyLlama 1.1B Chat 모델을 사용하여 응답을 생성하는 함수
    """
    # 사용자 콘텐츠 구성
    ...
    outputs = pipe(
        ...
        do_sample=True,         # 샘플링 사용 (약간의 창의성 부여)
...
question = "체중 1kg 기준으로 파워요가 운동 시 소비하는 에너지가 궁금해요."
response = extract_response_tinyllama(question)
print(response)

question = "스마트금융과는 어디에 위치하나요?"
...
if inColab == True:
    ...
    savePath = f"./models/tune_{now_str}"
    trainer.save_model(savePath)
